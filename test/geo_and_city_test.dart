...
void main() {
  group('GeoPoint datetime', () {
    test('toJson форматирует datetime: ART wall-clock + суффикс +0000', () {
      // 15 июня 2024, 07:30:00 UTC = 04:30:00 по аргентинскому времени (ART, UTC-3).
      // Требование бэкенда: wall-clock = ART, суффикс строки = +0000.
      final utc = DateTime.utc(2024, 6, 15, 7, 30, 0);
      final p = GeoPoint(lat: 55.75, lng: 37.62, datetime: utc);

      final json = p.toJson();

      expect(json['datetime'], '2024-06-15 04:30:00+0000');
      expect(json['lat'], 55.75);
      expect(json['lng'], 37.62);
      expect(json['nearest_city'], '');
    });

    test('toJson сдвигает произвольный локальный момент в ART', () {
      // Локальное 2024-01-15 22:05:03 эквивалентно какому-то UTC-моменту;
      // wall-clock в строке должен быть UTC-3 (ART), суффикс +0000.
