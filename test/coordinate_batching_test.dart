...
void main() {
  group('geoPointsFromActions', () {
      ...
      final actions = [
        PendingAction(
          ...
          payload: {
            'lat': 55.75,
            'lng': 37.61,
            // UTC wall-clock с псевдо-смещением -0300 (формат GeoPoint.toJson):
            // 07:00 — это UTC, истинный момент 07:00 UTC. Парсер вычитает
            // 3 часа, компенсируя буквальное чтение -0300 (07:00-(-3)=10:00 UTC).
            'datetime': '2026-07-15 07:00:00-0300',
            'nearest_city': 'Москва',
          },
        ),
      ];
    ...
    test('цикл очередь→повтор не накапливает сдвиг', () {
      // Защита регрессии: строка из payload (UTC + псевдо--0300) должна
      ...
      final stored = '2026-07-15 07:00:00-0300';
      final actions = [
        PendingAction(
          type: PendingActionType.coordinates,
          payload: {
